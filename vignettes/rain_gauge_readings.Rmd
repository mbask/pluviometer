...
get_water_balance <- function(water_balance_l, 
  ...
  is_recharged <- new_water_balance_l < 0
  if (is_recharged)  {
...
Then we walk the `water_use_tbl` data frame to estimate the water balance on a dayly basis:
```{r}
for (i in seq(1, nrow(water_use_tbl), by = 1)) {
 
  if (i == 1) {
      prior_water_balance_l <- water_use_tbl$water_balance_l[i]
  } else {
      prior_water_balance_l <- water_use_tbl$water_balance_l[i - 1]
  } 

  tmp_df <- get_water_balance(
      prior_water_balance_l, 
      water_use_tbl$tank_rain_charge_l[i], 
      water_use_tbl$daily_water_usage_l[i], 
      tank_volume_l)
...
The jaw-teeth plot of the dayly water balance in the tank reveals the grim role of rainfall in the summer season as far as water budget for irrigation is concerned:
```{r fig.width=7}
water_use_tbl %>% 
