...
precipitazione %<>%
  mutate(
    ...
    , precipitazione_mm     = get_precipitation_measure(volume_acqua_ml, fattore_pluviometrico) # ml o l/m2
    , ricarica_cisterna_l   = get_tank_charge(precipitazione_mm, cfg$superficie_tetto, cfg$perdita_acqua_per_evaporazione_tetto) # l
  )

monthly_precipitation <- precipitazione %>%
  mutate(year = year(data_dmy), month = month(data_dmy, label = TRUE)) %>%
  group_by(month, year) %>%
  select(precipitazione_mm, ricarica_cisterna_l) %>%
